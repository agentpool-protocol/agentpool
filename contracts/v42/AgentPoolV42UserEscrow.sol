...
} from "../v4/interfaces/IAgentPoolV41.sol";

/// @notice Existing-token external-job escrow. Keeper compensation is supplied
///         by the buyer and never mints new tAPOOL.
contract AgentPoolV42UserEscrow is ReentrancyGuard {
    ...
    event JobRefunded(uint256 indexed jobId, address indexed keeper);
    ...
    function fundJob(
        ...
        token.safeTransferFrom(
            msg.sender,
            address(this),
            uint256(budget) + keeperFee
        );
        emit JobFunded(jobId, msg.sender, worker, budget, keeperFee);
    }
    ...
    function resolve(
        ...
        token.safeTransfer(msg.sender, job.keeperFee);
        emit JobSettled(jobId, passed, msg.sender);
    }
