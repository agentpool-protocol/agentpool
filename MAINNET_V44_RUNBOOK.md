...
contract source commit. The evidence-pipeline commit is recorded separately
and must remain unchanged for the full observation window. The older v4.3
public deployment is compatibility evidence only and can never satisfy this
gate. Eligibility requires at least 90 days, 180 distinct RPC-verified
...
`outputs/v44-public-testnet-observations.json`, historical contract source
evidence, the current evidence-pipeline commit, and two Base Sepolia RPCs are
all present. Only
