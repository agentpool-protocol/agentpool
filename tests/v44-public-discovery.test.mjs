...
test("v4.4 public discovery exposes the exact deployed read-only alpha", () => {
  ...
  assert.match(discovery, /0\.14\.2-provenance-verified-alpha/u);
  ...
  assert.match(publicStatus, /TWO_RUNNER_TESTNET_VERIFIED/u);
  assert.match(publicStatus, /twoRunnerCampaign/u);
  assert.doesNotMatch(
    publicStatus,
    /SECOND_ENGINEERING_RUNTIME_REPORT_NOT_YET_VERIFIED/u,
  );
