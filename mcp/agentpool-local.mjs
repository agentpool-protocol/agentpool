...
const V41_TOKEN_ADDRESS = "0x74e69580d822573ad15148e6475f259fedc46ebb";
const V41_TOKEN_DECIMALS = 18;
const V41_TOKEN_ABI = [
  {
    type: "function",
    name: "balanceOf",
...
function createServer() {
  ...
  server.registerTool(
    "agentpool_transfer_test_tokens",
    {
      ...
      description:
        "Transfer tAPOOL from this computer's persistent AgentPool test wallet to a supplied Base Sepolia address. The local MCP signs without revealing the private key.",
      inputSchema: z
        .object({
          ...
          confirmation: z.literal("TRANSFER BASE SEPOLIA TEST TAPOOL"),
        })
        .strict(),
      annotations: {
        readOnlyHint: false,
    ...
    async ({ recipient, amountTapool }) => {
      ...
      if (
        destination.toLowerCase() === ZERO_ADDRESS ||
        destination.toLowerCase() === account.address.toLowerCase()
      ) {
        return textResult(
          {
            error: "INVALID_TEST_TOKEN_RECIPIENT",
            message:
              "The recipient must be a different nonzero Base Sepolia address.",
          },
          true,
        );
      }
      ...
      const { publicClient, walletClient } = await requireBaseSepolia(account);
      const [tokenBytecode, testEth, senderBefore, recipientBefore] =
        await Promise.all([
          publicClient.getBytecode({ address: V41_TOKEN_ADDRESS }),
          publicClient.getBalance({ address: account.address }),
          publicClient.readContract({
